...
Use the `edit_file` tool to write the summary to `~/.nkd-agents/sessions/<unix_timestamp>_<descriptive-1-3-word-phrase>.md` (use the actual current unix timestamp; derive the short phrase from the session's main topic, lowercase, underscore-separated). Then confirm the path to the user.
