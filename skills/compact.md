...
Use the `edit_file` tool to write the summary to `~/.nkd-agents/sessions/<unix_timestamp>_<descriptive_1_3_word_phrase>.md` (use the actual current unix timestamp; derive the short phrase from the session's main topic, lowercase, underscore-separated). Then confirm the path to the user.
